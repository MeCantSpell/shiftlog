...
def test_summary_worker_shift_pay_positive(client: TestClient):
    
    get_id_response = client.post("/workers", json={"name": "Jamie Lee",
                                  "role": "Cook",
                                  "pay":20})
    body = get_id_response.json()
    worker_id = body["id"]
    ...
    assert body["hourly_pay"] == 20
    assert body["average_shift_wage"] == 100
    assert body["worker_wages"] == 200
    
def test_summary_worker_shift_pay_zero(client: TestClient):
    ...
    body = response.json()
    assert body["worker_id"] == worker_id
    assert body["hourly_pay"] == 0
    assert body["average_shift_wage"] == 0
    assert body["worker_wages"] == 0

def test_summary_worker_shift_pay_null(client: TestClient):
